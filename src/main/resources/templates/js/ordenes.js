...
function showOrders(response) {
    ...
    for (i = 0; i < response.length; i++) {
        ...
        table += "<td> La orden de Pedido se encuentra: " + response[i].status + "</td>";
...
function showOrdersDetail(dato) {
    ...
    table += "<tr><td colspan=4 ><center><strong>Listado de Pedido</strong></center></td></tr>";
    table += "<tr>";
...
$("#btn_post_update").click(function() {
    ...
    $.when(jqueryPUT(url, data)).done(function() { // AJAX => ASINCRONO
        alert("El estado de la orden se ha actualizado correctamente!");
        closeForm();
